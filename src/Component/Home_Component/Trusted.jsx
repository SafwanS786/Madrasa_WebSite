...
export default function Trusted() {
  ...
  return (
    <div className="text-center px-4 sm:px-4 md:px-6 lg:px-8 xl:px-24 2xl:px-32">
      <div
      ...
      <h1 className="text-2xl sm:text-3xl md:text-5xl lg:text-5xl xl:text-6xl 2xl:text-6xl font-bold mt-8 mb-6 text-[#1A1A1A] leading-tight font-sen">
        Trusted by
        <span className="block bg-gradient-to-r from-[#126F77] to-[#EB6319] bg-clip-text text-transparent">
          Modern Madrasas
        </span>
      </h1>
      <p className="text-base md:text-xl lg:text-xl xl:text-2xl 2xl:text-2xl text-[#6B6B6B] max-w-3xl mx-auto leading-relaxed">
        Join hundreds of forward-thinking institutions that have revolutionized
        their management with our cutting-edge platform.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 gap-6 mt-12 px-4 sm:px-6 md:px-8 lg:px-4 xl:px-16 2xl:px-20">
        {/* Updated: Added md:grid-cols-2 and explicit xl/2xl:grid-cols-3 for clarity */}
        {data.map((item, index) => (
          <div
            className="text-card-foreground flex flex-col gap-6 rounded-xl group border
             border-gray-200 bg-white relative p-2 sm:p-3 md:p-4 lg:p-4 xl:p-8 2xl:p-8 text-center hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
            key={index}
          >
            {/* Updated: Added responsive padding for smaller screens */}
            <div className="p-2 sm:p-4 md:p-2 lg:p-2 xl:p-8 2xl:p-8 text-center">
              <div className="mb-2 md:mb-4 lg:mb-4">
                <div
                  className="w-13 h-13 sm:w-16 sm:h-16 md:w-17 md:h-17 lg:w-18 lg:h-18 xl:w-20 xl:h-20 2xl:w-20 2xl:h-20 bg-gradient-to-r rounded-2xl flex items-center justify-center mx-auto shadow-lg group-hover:shadow-xl group-hover:scale-105 transition-all duration-300"
                  style={{
                ...
                >
                  ...
                  <item.icon className="w-5 h-5 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-10 lg:h-10 xl:w-10 xl:h-10 2xl:w-10 2xl:h-10 text-white" />
                  {/* Updated: Added responsive icon sizes */}
                </div>
              </div>
              <h2 className="text-base sm:text-base md:text-xl lg:text-xl xl:text-2xl 2xl:text-2xl font-bold text-[#1A1A1A] group-hover:text-[#126F77] transition-colors duration-300">
                {item.Title}
              </h2>
              <p className="text-sm  md:text-lg lg:text-base xl:text-xl 2xl:text-xl text-[#6B6B6B] mt-4 leading-relaxed">
                {item.Desc}
              </p>
            </div>
          </div>
